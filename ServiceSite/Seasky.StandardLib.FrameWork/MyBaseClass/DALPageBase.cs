...
namespace SeaSky.StandardLib.MyBaseClass
{
    ...
    /// <typeparam name="ViewModel">返回的ViewModel</typeparam>
    public abstract class DALPageBase<Model, OutputModel, ViewModel> : DALBase<Model, OutputModel, ViewModel>
        ...
        where ViewModel : class, Model, new()
    {
        ...
        public virtual PageModel<OutputModel> ListPage(Model model)
        {
            ...
            PageModel<OutputModel> result = new PageModel<OutputModel>();
            ParmCollection parms = this.Table.PrepareConditionParms(model, this.IsLikeMode);
            string requirSql = string.Format(this.Table.Select, parms.WhereSql, string.Empty);
        ...
        public virtual PageModel<ViewModel> ListViewPage(ViewModel model)
        {
            ...
            PageModel<ViewModel> result = new PageModel<ViewModel>();
            ParmCollection parms = this.Table.PrepareConditionParms(model, this.IsLikeMode);
            string requirSql = string.Format(this.Table.Select, parms.WhereSql, string.Empty);
            PageInfo pageInfo = new PageInfo(requirSql, this.OrderBy, model.PageSize ?? 10, model.PageNO ?? 1);
            result.ListData = ListView(pageInfo.PageSQL, parms);
            result.PageNO = model.PageNO ?? 1;
            result.PageSize = model.PageSize ?? 10;
            using (IDataReader rdr = Internal_DataHelper.ExecuteReader(pageInfo.PageCountSQL, parms))
            {
